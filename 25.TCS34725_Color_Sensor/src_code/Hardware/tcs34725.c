...
void tcs3472_test( void ) 
{
	...
	char buf[50] = {0};
	uint16_t __r, __g, __b, __c/*, colorTemp, lux*/;
	HAL_GPIO_WritePin(LED_GPIO_Port, LED_Pin, GPIO_PIN_SET);
	tcs3272_init();
	//获取原始数据
	getRawData(&__r, &__g, &__b, &__c);
	...
	//printf("r:%d g:%d b:%d colorTemp=%d  lux=%d\n",R,G,B,colorTemp,lux);
	uint16_t color565 = convertRGB24toRGB565(R,G,B);
	printf("color565 = 0x%x\n",color565);
//	LCD_Clear(color565);
	//LCD_Draw_ColorRect(0,0,160,160,color565);
	memset(buf,0,50);
	count++;
	if(count >= 15)
	{
		...
		LCD_ShowCharStr(0,200,240,buf,BLACK,WHITE,24);
	}
	LCD_Fill(0,0,240,190,color565);
}
