...
import com.hnj.printtest_serialport.view.PopMenu;
import com.hnj.printtest_serialport.view.PopMenu.OnItemClickListener;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		save_set_bt.setOnClickListener(new OnClickListener() {			
			...
			public void onClick(View arg0) {
				...
				if(com_name.equals("")||baud_rate.length()<=0)
				{
					Toast_Util.ToastString(getApplicationContext(), R.string.please_enter_a_serial_port_name);//请输入串口名称
				}else {
					
					boolean isopen=false;
					if(!baud_rate.equals("")&&!com_name.equals(""))
					{
						isopen=MainActivity.sf.OpenPort(com_name, baud_rate);
					}else {
						isopen=MainActivity.sf.OpenPort("ttyS0", "115200");  
					}	
					if(isopen)
					{
